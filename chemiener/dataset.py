...
class NERDataset(Dataset):
    def __init__(self, args, data_file, split="train"):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.args.roberta_checkpoint, cache_dir=self.args.cache_dir
        )  # BertTokenizerFast.from_pretrained('allenai/scibert_scivocab_uncased')
    ...
    def __getitem__(self, idx):
        text_tokenized = self.tokenizer(
            self.data[str(idx)]["text"],
            truncation=True,
            max_length=self.args.max_seq_length,
        )
        if len(text_tokenized["input_ids"]) > 512:
        ...
        return (
            ...
            self.align_labels(
                text_tokenized,
                self.data[str(idx)]["entities"],
                len(self.data[str(idx)]["text"]),
            ),
            self.align_labels(
                text_tokenized_untruncated,
                self.data[str(idx)]["entities"],
                len(self.data[str(idx)]["text"]),
            ),
        )
        ...
        for entity in entities:
            for span in entities[entity]["span"]:
                for i in range(span[0], span[1]):
                    char_to_class[i] = self.class_to_index[
                        ("B-" if i == span[0] else "I-") + str(entities[entity]["type"])
                    ]
