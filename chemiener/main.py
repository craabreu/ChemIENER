...
def get_args(notebook=False):
    ...
    parser.add_argument(
        "--scheduler", type=str, choices=["cosine", "constant"], default="cosine"
    )
    ...
    parser.add_argument(
        "--save_mode", type=str, default="best", choices=["best", "all", "last"]
    )
...
class ChemIENERecognizer(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, logits = self.model(
            input_ids=sentences, attention_mask=masks, labels=refs
        )
    ...
    def on_validation_epoch_end(self):
        ...
        untruncateds = [
            [
                index_to_class[int(label.item())]
                for label in sentence
                if int(label.item()) != -100
            ]
            for batched in untruncateds
            for sentence in batched
        ]
        ...
        output = {
            "sentences": [
                [
                    int(word.item())
                    for (word, label) in zip(sentence_w, sentence_l)
                    if label != -100
                ]
                for (batched_w, batched_l) in zip(sentences, labels)
                for (sentence_w, sentence_l) in zip(batched_w, batched_l)
            ],
            "predictions": [
                [
                    index_to_class[int(pred.item())]
                    for (pred, label) in zip(sentence_p, sentence_l)
                    if label != -100
                ]
                for (batched_p, batched_l) in zip(predictions, labels)
                for (sentence_p, sentence_l) in zip(batched_p, batched_l)
            ],
            "groundtruth": [
                [
                    index_to_class[int(label.item())]
        ...
        if self.trainer.is_global_zero:
            if not self.args.no_eval:
                ...
                predictions = [
                    preds + ["O"] * (len(full_groundtruth) - len(preds))
                    for (preds, full_groundtruth) in zip(
                        output["predictions"], untruncateds
                    )
                ]
                all_metrics = metric.compute(
                    predictions=predictions, references=untruncateds
                )
            ...
            with open(
                os.path.join(self.trainer.default_root_dir, f"prediction_{name}.json"),
                "w",
            ) as f:
                json.dump(output, f)
    ...
    def configure_optimizers(self):
        ...
        scheduler = get_scheduler(
            self.args.scheduler, optimizer, num_warmup_steps, num_training_steps
        )
        return {
            "optimizer": optimizer,
...
class ModelCheckpoint(pl.callbacks.ModelCheckpoint):
    def _get_metric_interpolated_filepath_name(
        self, monitor_candidates, trainer, del_filepath=None
    ) -> str:
...
def main():
    ...
    if args.do_train:
        ...
        model = ChemIENERecognizer.load_from_checkpoint(
    ...
    checkpoint = ModelCheckpoint(
        monitor="val/score", mode="max", save_top_k=1, filename="best", save_last=True
    )
    ...
    if args.do_train:
        trainer.num_training_steps = (
            math.ceil(
                ...
                / (args.batch_size * args.gpus * args.gradient_accumulation_steps)
            )
            * args.epochs
        )
        model.eval_dataset = dm.val_dataset
        ckpt_path = (
            os.path.join(args.save_path, "checkpoints/last.ckpt")
            if args.resume
            else None
        )
        trainer.fit(model, datamodule=dm, ckpt_path=ckpt_path)
        model = ChemIENERecognizer.load_from_checkpoint(
            checkpoint.best_model_path, args=args
        )
