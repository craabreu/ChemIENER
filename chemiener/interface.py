...
class ChemNER:
    def __init__(self, model_path, device=None, cache_dir=None):
        ...
        self.index_to_class = {
            self.class_to_index[key]: key for key in self.class_to_index
        }
    ...
    def _get_args(self, cache_dir):
        ...
        parser.add_argument(
            ...
            type=str,
            help="which roberta config to use",
        )
        ...
        parser.add_argument(
            ...
            default="chemdner",
            type=str,
            help="which corpus should the tags be from",
        )
    ...
    def predict_strings(self, strings: List, batch_size=8):
        ...
        def prepare_output(char_span, prediction):
            ...
            while i < len(char_span):
                if prediction[i][0] == "B":
                    toreturn.append(
                        (prediction[i][2:], [char_span[i].start, char_span[i].end])
                    )

                elif len(toreturn) > 0 and prediction[i][2:] == toreturn[-1][0]:
                    toreturn[-1] = (
                        toreturn[-1][0],
                        [toreturn[-1][1][0], char_span[i].end],
                    )
        ...
        for idx in range(0, len(strings), batch_size):
            ...
            batch_strings_tokenized = [
                (
                    self.dataset.tokenizer(s, truncation=True, max_length=512),
                    torch.Tensor([-1]),
                    torch.Tensor([-1]),
                )
            ...
            predictions = (
                self.model(
                    input_ids=sentences.to(device), attention_mask=masks.to(device)
                )[0]
                .argmax(dim=2)
                .to("cpu")
            )
            ...
            for j, sentence in enumerate(sentences_list):
                to_add = [
                    batch_strings_tokenized[j][0].token_to_chars(i)
                    for i, word in enumerate(sentence)
                    if len(
                        self.dataset.tokenizer.decode(
                            int(word.item()), skip_special_tokens=True
                        )
                    )
                    > 0
                ]
            ...
            class_predictions = [
                [
                    self.index_to_class[int(pred.item())]
                    for (pred, word) in zip(sentence_p, sentence_w)
                    if len(
                        self.dataset.tokenizer.decode(
                            int(word.item()), skip_special_tokens=True
                        )
            ...
            output += [
                prepare_output(char_span, prediction)
